...
%
% ElSpec_iqt is a variant that does recursive division in time as long as
% that is Akaike-information-criteria-justified, and runs the optimization
% with fminsearchbnd
%
...
%
% IV 2017, 2018, BG 2020-2024, this version builds on ElSpec.
%
...
% number of time-slices to proceed on each time-step
defaultNstepmin = 1;
checkNstepmin = @(x) (isnumeric(x));

% number of time-slices to proceed on each time-step
defaultNchunk = 128;
checkNchunk = @(x) (isnumeric(x) & length(x)==1 & all(x>0));

% save the  large flux covariance matrix?
defaultDoiecov = false;
checkDoiecov = @(x) ((isnumeric(x)|islogial(x))&length(x)==1);
...
if numel(out.nstepmin) == 1
    out.nstepmin = out.nstepmin.*ones(1, numel(out.ts))
end
...
%  save(savename,'ne00')
  ...
  AICFull = AICc( pp, ...
                  ...
                  numel(ne), ...
                  Directives.ErrType, ...
                  Directives.ErrWidth ) + mean(nstepmin);
  if numel(dt) == 1 
    ...
    AIChalves = AICc( pp, ...
                      ...
                      Directives.ErrType, ...
                      Directives.ErrWidth ) + mean(nstepmin(1:idxPartition2)) + mean(nstepmin((idxPartition2+1):end));
    ...
    AICthirds = AICc( pp, ...
                      ...
                      Directives.ErrType, ...
                      Directives.ErrWidth ) + mean(nstepmin(1:idxPartition3)) + mean(nstepmin((idxPartition3+1):(2*idxPartition3))) + mean(nstepmin((idxPartition3+1):(2*idxPartition3)));
    end
    ...
    elseif AICthirds < AICFull & AICthirds <= AIChalves
      ...
      [ne2_3,neEnd2_3,Ie2_3,polycoefs2_3,best_order2_3,n_params2_3,exitflag2_3,nSteps2_3,idx_out2_3] = recurse_AICfit(ne2_3,...
                                                        ...
                                                        [], ... % was: stdprior,...
                                                        Ie1_3(:,end),...
                                                        nstepmin((idxPartition3+1):(2*idxPartition3)),...
                                                        Directives);
      [AIC3_3,polycoefs3_3,b_o3_3,n_params3_3,ne3_3,neEnd3_3,Ie3_3,exitflag3_3] = AICcFitParSeq(pp(:,(2*idxPartition3+1):end),...
                                                        ppstd(:,(2*idxPartition3+1):end),...
                                                        alpha(:,(2*idxPartition3+1):end),...
      ...
      [ne3_3,neEnd3_3,Ie3_3,polycoefs3_3,best_order3_3,n_params3_3,exitflag3_3,nSteps3_3,idx_out3_3] = recurse_AICfit(ne3_3,...
                                                        ...
                                                        [], ... % was: stdprior,...
                                                        Ie2_3(:,end),...
                                                        nstepmin((2*idxPartition3+1):end),...
                                                        Directives);
      % and conquer!
