...
%     % start indices of profiles
    ...
    if strcmpi(tres,'best')
      for iDX = 1:numel(i_end)
        ...
        % this is a problem when there are lacking power-profile samples,
        % then curr_set will be empty. 
        % for the case when there is only one power-profile we should be
        % able to rescue the situation by setting the current profiles to
        % nans and then inpaint_nan it at the end. This is not ideal,
        % because it will get confused when there are more than one
        % power-profile and we cannot for sure tell which power-profile
        % we're looking at
        if isempty(curr_set)
          h{1}(:,idx_profs(1)) = nan;
          pp{1}( : ,idx_profs(1)) = nan;
          ppstd{1}( :,idx_profs(1)) = nan;
          idx_profs(1) = idx_profs(1)+1;
        else
          h{curr_set}(:,idx_profs(curr_set)) = r_pprange(curr_idx)*sind(r_el);
...
if strcmpi(tres,'best')
  ...
  wH = H;
  for i_set = 1:numel(nh_all)
    if any(isnan(pp{i_set}(:)))
      pp{i_set}    = inpaint_nans(pp{i_set});
      ppstd{i_set} = inpaint_nans(ppstd{i_set});
      h{i_set}     = inpaint_nans(h{i_set});
    end
    for i_z = 1:nh_all(i_set)       
